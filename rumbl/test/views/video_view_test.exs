defmodule Rumbl.VideoViewTest do
  ...
  test "renders index.html", %{conn: conn} do
    videos = [%Rumbl.Video{id: "1", title: "dogs", category: %Rumbl.Category{id: "1", name: "Action"}},
              %Rumbl.Video{id: "2", title: "cats", category: %Rumbl.Category{id: "1", name: "Action"}}]
    content = render_to_string(Rumbl.VideoView, "index.html", conn: conn, videos: videos)

    assert String.contains?(content, "Listing videos")
    for video <- videos do
      assert String.contains?(content, video.title)
      assert String.contains?(content, video.category.name)
    end
  end
